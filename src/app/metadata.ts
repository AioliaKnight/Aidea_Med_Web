...
export const medicalServicesFaq = createFaqSchema([
  {
    question: '醫療行銷與一般行銷有何不同？',
    answer: '醫療行銷需要兼顧專業性與法規遵循，同時考慮醫病關係的特殊性。與一般行銷不同，醫療行銷更強調建立信任、專業權威與長期關係，而非短期轉換。我們的策略著重在以教育為基礎的內容行銷、專業形象塑造與符合醫療法規的溝通方式，確保行銷內容既能吸引患者，又能維護醫療專業形象。'
  },
  {
    question: '如何衡量醫療行銷的投資報酬率？',
    answer: '我們使用多維指標評估醫療行銷投資報酬率：首先是量化指標，包括網站流量成長、諮詢轉換率、新患者數量增加、預約數成長與自費項目銷售增長；其次是質化指標，包括品牌知名度提升、患者信任度增強、醫師專業形象改善與患者回饋評價。我們會根據您診所的具體目標，設定適合的KPI指標，並提供定期報告，讓您清楚掌握行銷活動的實際成效。'
  },
  {
    question: '小型診所的行銷預算應如何分配？',
    answer: '小型診所的行銷預算應集中在最能觸及目標患者的渠道。我們建議：30-40%投入數位媒體廣告(Google、社群平台)；20-25%用於診所網站優化與SEO；15-20%發展有價值的醫療內容；10%投入電子郵件行銷系統建立患者關係；5-10%用於線下推廣活動。對於初創診所，建議先投入於品牌建立與本地SEO；成熟診所則可更多著重於患者關係管理與口碑行銷。我們會根據診所的發展階段與目標人群，調整最適合的預算分配比例。'
  },
  {
    question: '如何在遵守醫療法規的前提下做有效的行銷？',
    answer: '合規行銷是我們的首要原則。我們策略包括：嚴格遵守衛福部醫療廣告相關規範；使用教育性內容而非誇大宣傳；展示真實案例與患者評價(經適當授權)；明確標示療程風險與恢復期資訊；避免價格戰與低價促銷；強調醫師專業背景與診所設備的實際優勢。我們的行銷團隊定期接受醫療法規培訓，並與法規顧問合作，確保所有行銷內容在創意與合規之間取得最佳平衡。'
  }
])
